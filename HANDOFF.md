...
最近一次迭代重心：**公开页「🔥 热点统计」看板** + **采集源扩充（RSS / 行业媒体 / 公众号品牌词）** + **公开页"每日关注"栏目** + **内容质量门禁（隔离区）** + **AI 摘要 120~200 字硬约束**。
...
1. **公开页「🔥 热点统计」看板** —— 新增 `hot_stats.py` 聚合层，30 天趋势/厂商-行业-来源分布/相关度/标签云/领域热词/高相关头条；每晚采集后自动写快照（`config` 表 key=`hot_stats:YYYY-MM-DD`）；前端 hash 路由 `#hot`，支持历史快照日期切换 + 手动刷新
2. **采集数据源扩充** —— 新增 RSS/Atom 直采（`feedparser`，`config.RSS_SOURCES`：中新网科技/人民网科技/新华网科技，按关键词过滤）；新增行业媒体与公众号品牌词搜索阶段（`INDUSTRY_MEDIA_QUERIES`，20 条覆盖泰伯网/36氪/智东西/机器之心/微信公众号等）；`collect_once()` 由三阶段扩展为五阶段（官网→厂商搜索→行业搜索→媒体/公众号搜索→RSS）
3. **公开页"每日关注"栏目** —— 五维度（行业动态/产品/技术/市场/关注点）当日聚合，`daily_focus.py` 生成快照存 `config` 表，采集完成后自动刷新
