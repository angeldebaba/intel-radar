...
最近一次迭代重心：**公开页新增「🌐 行业观察」全局看板（行业研究视角）** + **原「热点统计」更名为「📊 情报看板」**（基于本站抓取数据的热度聚合，与行业观察互补）+ **采集源扩充（RSS / 行业媒体 / 公众号品牌词）** + **公开页"每日关注"栏目** + **内容质量门禁（隔离区）** + **AI 摘要 120~200 字硬约束**。
...
2. **原「热点统计」更名为「📊 情报看板」** —— 仍由 `hot_stats.py` 聚合**本站抓取情报**的热度分布/趋势/热词/头条，与"行业观察"形成互补：一个看内部数据热度，一个看行业全局
...
4. **公开页"每日关注"栏目** —— 五维度（行业动态/产品/技术/市场/关注点）当日聚合，`daily_focus.py` 生成快照存 `config` 表，采集完成后自动刷新
